...
if [[ ! -s "$PGDATA/PG_VERSION" ]]; then
    ...
    chmod 0600 ~/.pgpass
    until ping -c 1 -W 1 postgres_primary; do
        echo "Waiting for primary to ping..."
        sleep 2s
    done

    until pg_basebackup -h postgres_primary -D $PGDATA -U $POSTGRES_REP_USER -vP -W; do
...
standby_mode = on
primary_conninfo = 'host=postgres_primary port=5432 user=$POSTGRES_REP_USER password=$POSTGRES_REP_PASSWORD'
