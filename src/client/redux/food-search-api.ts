...
function* searchFoodItemByUpcSaga(): Generator {
  yield takeEvery(FoodSearchApiActions.START_SEARCH_FOOD_ITEMS_BY_UPC, function* (action: PayloadAction): Generator {
    ...
    try {
      ...
      const existingFoodItem: IFoodItem = yield call(() =>
        axios.get(`/api/food-items/by-upc/${upc}`).then((res) => mapFoodItemFromJson(res.data as IJsonObject)),
      );

      if (existingFoodItem) {
        yield all([put(setFoodItemsByUpc(upc, [existingFoodItem])), put(setUpcSearchBusy(false))]);
      } else {
        const tescoFoodItems: IFoodItem[] = yield call(() =>
          axios
        ...
        const foodItems = [...tescoFoodItems, ...nutritionixFoodItems];

        yield all([put(setFoodItemsByUpc(upc, foodItems)), put(setUpcSearchBusy(false))]);
      }
    } catch (err) {
...
function* foodSearchApiSagas(): Generator {
  yield all([searchFoodItemByUpcSaga(), searchFoodItemByKeywordSaga()]);
}
