...
function validateTarget(target: Partial<ITarget>): ITargetValidationResult {
  ...
  const countOfRemainders = [target.carbohydratesTargetMode, target.fatTargetMode, target.proteinTargetMode].filter(
    (m) => m === TargetMode.REMAINDER_OF_CALORIES,
  ).length;
  if (countOfRemainders > 1) {
    ...
    const caloriesFromMacros =
      macroSummary.targetCaloriesFromCarbohydrates +
