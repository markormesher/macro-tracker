...
function generateMacroSummary(
	...
	const totalTargetCaloriesFromMacros = targetCaloriesFromCarbohydrates
	...
	const totalCaloriesFromCarbohydrates = nutritionAmounts
			...
			.reduce(reduceSum, 0);
	const totalCaloriesFromFat = nutritionAmounts
			.map((a) => a.fat * CALORIES_PER_G_FAT)
			.reduce(reduceSum, 0);
	const totalCaloriesFromProtein = nutritionAmounts
			.map((a) => a.protein * CALORIES_PER_G_PROTEIN)
			.reduce(reduceSum, 0);
	const totalCarbohydrates = nutritionAmounts
			.map((a) => a.carbohydrates)
			.reduce((a, b) => a + b, 0);
...
function calculateTargetCalories(
		totalCalories: number, targetMode: TargetMode, targetValue: number, caloriesPerG: number, bodyWeight: number,
): number {
	switch (targetMode) {
