...
import { IDiaryEntry, validateDiaryEntry } from "../models/IDiaryEntry";
...
function getDiaryEntryQueryBuilder(): SelectQueryBuilder<DbDiaryEntry> {
  return DbDiaryEntry.createQueryBuilder("diary_entry")
...
async function saveDiaryEntry(diaryEntryId: string, values: IDiaryEntry): Promise<DbDiaryEntry> {
  ...
  return getDiaryEntry(diaryEntryId).then((diaryEntry) => {
...
export { getDiaryEntryQueryBuilder, getDiaryEntry, getDiaryEntriesForDate, saveDiaryEntry, deleteDiaryEntry };
