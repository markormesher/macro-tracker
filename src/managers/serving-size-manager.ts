...
import { IServingSize, validateServingSize } from "../models/IServingSize";
...
function getServingSizeQueryBuilder(): SelectQueryBuilder<DbServingSize> {
  return DbServingSize.createQueryBuilder("serving_size");
}
...
async function saveServingSize(values: IServingSize, force = false): Promise<DbServingSize> {
  ...
  return getServingSize(servingSizeId).then((servingSize) => {
...
export { getServingSizeQueryBuilder, getServingSize, saveServingSize };
